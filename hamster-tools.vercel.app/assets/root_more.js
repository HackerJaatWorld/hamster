...
function stone() {
    ...
    const handleGenerate = async () => {
        const numberOfCodes = parseInt(document.getElementById("stone_num_of_code").value);
        if (isNaN(numberOfCodes)) {
            tt("Please enter a number");
            return;
        } else if (numberOfCodes < 1 || numberOfCodes > 30) {
        ...
        try {
            ...
            setCurrentStatus("wait");
            setProgress(0);
            const newCodes = await Promise.all(Array.from({ length: numberOfCodes }, () => stone3.generate()));
            setCodes(newCodes);
            setCurrentStatus("done");
            setProgress(100);
            const generatedCodeCount = parseInt(localStorage.getItem('generatedCodeCount') || '0');
